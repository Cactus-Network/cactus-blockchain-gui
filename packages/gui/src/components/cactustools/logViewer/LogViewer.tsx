import { Flex, useShowError } from '@cactus-network/core';
...
export default function LogViewer({ pageSize = 1000 }: LogViewerProps) {
  ...
  const checkLogFile = useCallback(async (): Promise<LogFileInfo> => {
    try {
      const info = await window.cactusLogsAPI.getInfo();
      if ('error' in info) {
  ...
  const loadLogsData = useCallback(
    async (forceReload = false) => {
      ...
      try {
        ...
        const result = await window.cactusLogsAPI.getContent();
        if ('error' in result) {
  ...
  const handleCustomPathSubmit = useCallback(async () => {
    try {
      await window.cactusLogsAPI.setPath();
      const info = await checkLogFile();
      setFileInfo(info);
  ...
  const handleExport = useCallback(async () => {
    try {
      ...
      a.href = url;
      a.download = 'cactus-logs-export.log';
  ...
  const content = useMemo(() => {
    ...
    if (rawContent) {
      return (
        <MonospaceBox>
          {rawContent.split('\n').map((line, index) => (
            // eslint-disable-next-line react/no-array-index-key -- Using index is safe here as the array is stable and order won't change (requested by CactusMineJP)
        ...
        <Typography color="textSecondary">
          ...
          {fileInfo.debugInfo && (
            <>
              ...
              Debug Information:
              <br />
              Cactus Root: {fileInfo.debugInfo.cactusRoot}
              <br />
