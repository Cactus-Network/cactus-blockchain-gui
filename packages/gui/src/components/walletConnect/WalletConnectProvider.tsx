...
const log = initDebug('cactus-gui:walletConnect');
...
function processError(error: Error) {
  if (error.message.includes('No matching key')) {
    console.info('[cactus-gui:walletConnect] Pairing not found (stale key, safe to ignore):', error.message);
    return;
  }
...
function parsePair(topic: string, mainnet: boolean, proposal: SignClientTypes.EventArguments['session_proposal']) {
  ...
  const requiredCactus = requiredNamespaces?.cactus;
  const optionalCactus = optionalNamespaces?.cactus;

  if (!requiredCactus && !optionalCactus) {
    throw new Error('Missing required cactus namespace');
  }

  const chains = [...new Set([...(requiredCactus?.chains ?? []), ...(optionalCactus?.chains ?? [])])];

  const currentChain = mainnet ? 'cactus:mainnet' : 'cactus:testnet';
  ...
  const commands = [...new Set([...(requiredCactus?.methods ?? []), ...(optionalCactus?.methods ?? [])])];
