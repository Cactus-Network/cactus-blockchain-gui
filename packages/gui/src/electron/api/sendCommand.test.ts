...
describe('sendCommand', () => {
  it('registers once and reuses the daemon socket for subsequent commands', async () => {
    ...
    await expect(sendCommand('get_network_info', 'cactus_wallet')).resolves.toMatchObject({
      success: true,
      command: 'get_network_info',
    });
    await expect(sendCommand('get_logged_in_fingerprint', 'cactus_wallet')).resolves.toMatchObject({
      success: true,
  ...
  it('routes concurrent responses by request id', async () => {
    ...
    const first = sendCommand('first_command', 'cactus_wallet');
    const second = sendCommand('second_command', 'cactus_wallet');
  ...
  it('parses unsafe integer response values as native bigints', async () => {
    ...
    const response = await sendCommand<{ mojos: unknown }>('get_wallet_balance', 'cactus_wallet');
  ...
  it('stringifies bigint command values as JSON numbers', async () => {
    ...
    await sendCommand('send_bigint', 'cactus_wallet', {
  ...
  it('rejects daemon command errors', async () => {
    ...
    await expect(sendCommand('send_transaction', 'cactus_wallet')).rejects.toThrow('daemon rejected command');
  });
  ...
  it('rejects an in-flight command when the socket closes and reconnects on the next command', async () => {
    ...
    await expect(sendCommand('close_me', 'cactus_wallet')).rejects.toThrow('Connection closed before receiving response');
    await expect(sendCommand('after_close', 'cactus_wallet')).resolves.toMatchObject({ reconnected: true });
