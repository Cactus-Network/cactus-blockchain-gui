...
type AssetDisplayKind = 'cactus' | 'wallet' | 'cat' | 'nft' | 'unknown';
...
function offerSummaryAssetKinds(offerSummary: OfferSummaryForDisplay): AssetDisplayKinds {
  ...
  for (const assetId of Object.keys(offerSummary.requested)) {
    const parsedAssetId = assetId === 'cac' ? '1' : assetId;
...
async function parseWalletDeltaItem(
  ...
  if (key === 'cac') {
    throw new Error('CAC is not a valid asset ID');
  }

  if (assetKind === 'cactus' || key === '1') {
    return {
      kind: 'cac',
      amount: mojoToCactusLocaleString(value),
    };
  }
...
function formatAmountWithRoyalties(
  ...
  if (line.kind === 'cac') {
    return mojoToCactusLocaleString(totalAmount);
  }
...
async function walletDeltaToDisplay(
  ...
  return {
    ...
    fee: fee !== undefined ? mojoToCactusLocaleString(fee) : undefined,
  };
}

export async function parseCommandDisplay(command: string, params: Record<string, unknown>) {
  if (command === 'cactus_wallet.take_offer') {
  ...
  if (command === 'cactus_wallet.create_offer_for_ids') {
