...
describe('parseCommandDisplay', () => {
  ...
  it('displays raw create offer signs as spending and receiving wallet deltas', async () => {
    mockGetWalletInfos.mockResolvedValue({
      1: { name: 'Cactus', type: WalletType.STANDARD_WALLET },
      6: { name: 'Test CAT', type: WalletType.CAT },
    });

    await expect(
      parseCommandDisplay('cactus_wallet.create_offer_for_ids', {
        offer: {
          1: -10_000,
          6: 2_323_000,
        },
  ...
  it('fails closed when create-offer params use cac keys instead of numeric wallet IDs', async () => {
    mockGetWalletInfos.mockResolvedValue({});
    ...
    ).rejects.toThrow('CAC is not a valid asset ID');
  });

  it('normalizes take-offer cac summary keys to CAC display rows', async () => {
    mockGetWalletInfos.mockResolvedValue({});
    ...
    await expect(
      parseCommandDisplay('cactus_wallet.take_offer', {
        offer: 'offer1...',
      }),
    ).resolves.toMatchObject({
      walletDelta: {
  ...
  it('fails closed if a raw cac asset key reaches display parsing', async () => {
    const entriesSpy = jest
      .spyOn(Object, 'entries')
      .mockReturnValueOnce([['1', '-1']])
      .mockReturnValueOnce([['cac', 1n]])
      .mockReturnValueOnce([]);
