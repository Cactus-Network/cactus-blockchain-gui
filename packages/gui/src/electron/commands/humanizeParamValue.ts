...
function formatMojoCac(amount: unknown, networkPrefix?: string): string {
  const formatted = mojoToCactusLocaleString(amount as string | number);
...
export async function humanizeParamValue(
  ...
  if (humanize) {
    switch (humanize) {
      case 'mojo-to-cac':
        return formatMojoCac(value, networkPrefix);
      case 'mojo-to-cat':
