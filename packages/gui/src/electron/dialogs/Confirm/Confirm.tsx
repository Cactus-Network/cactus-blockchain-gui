...
export type DisplayWalletDeltaItem =
  | { kind: 'cac'; amount: string; amountWithRoyalties?: string }
...
function offerLineKey(line: DisplayWalletDeltaItem, index: number): string {
  if (line.kind === 'cac') return `cac-${line.amount}-${index}`;
...
function OfferLineRow({ line, networkPrefix }: { line: DisplayWalletDeltaItem; networkPrefix?: string }) {
  if (line.kind === 'cac') {
    ...
    return (
      <div>
        <div className="text-sm font-medium text-cactus-text">
          {line.amount} {networkPrefix ? networkPrefix.toUpperCase() : 'CAC'}
        </div>
        {line.amountWithRoyalties && (
          <div className="text-xs text-cactus-text-secondary">
            {i18n._(/* i18n */ { id: 'Total Amount with Royalties' })}: {line.amountWithRoyalties}{' '}
            {networkPrefix ? networkPrefix.toUpperCase() : 'CAC'}
          </div>
        )}
      </div>
    );
    ...
    return (
      <div>
        <div className="flex items-baseline gap-3">
          <span className="text-sm font-medium text-cactus-text">
            {line.amount}
            {line.symbol && <span className="ml-1 text-cactus-text-secondary">{line.symbol}</span>}
          </span>
          <span className="text-xs font-mono text-cactus-text-secondary truncate max-w-[55%]">
  ...
  if (line.kind === 'wallet') {
    return (
      <div>
        <div className="flex items-baseline gap-3">
          <span className="text-sm font-medium text-cactus-text">{line.amount}</span>
          <span className="text-xs font-mono text-cactus-text-secondary truncate max-w-[55%]">
  ...
  if (line.kind === 'unknown') {
    return (
      <div className="flex items-baseline gap-3">
        <span className="text-sm font-medium text-cactus-text">{i18n._(/* i18n */ { id: 'Unknown Asset' })}</span>
        <span className="text-xs font-mono text-cactus-text-secondary truncate max-w-[55%]">
          {shortenId(line.assetId)}
        </span>
        <span className="text-xs font-mono text-cactus-text-secondary">
    ...
    <div className="flex items-center gap-3">
      {line.previewUrl ? (
        <img
          src={line.previewUrl}
          alt=""
          className="shrink-0 w-10 h-10 rounded-md object-cover bg-cactus-card"
          referrerPolicy="no-referrer"
        />
      ) : (
        <div className="shrink-0 w-10 h-10 rounded-md bg-cactus-primary/15 text-cactus-primary flex items-center justify-center text-[10px] font-bold uppercase tracking-wider">
          NFT
        </div>
      )}
      <div className="flex-1 min-w-0">
        {line.name && <div className="text-sm font-medium text-cactus-text truncate">{line.name}</div>}
        <span className="text-xs font-mono text-cactus-text-secondary truncate block">{shortenId(line.nftId)}</span>
        {line.royaltyPercentage !== undefined && line.royaltyPercentage > 0 && (
          <div className="text-xs font-mono text-cactus-text-secondary truncate block">
...
function WalletDeltaSection({
  ...
  const feeUnit = networkPrefix ? networkPrefix.toUpperCase() : 'CAC';
  return (
    <section className="rounded-xl border border-cactus-border bg-cactus-card overflow-hidden divide-y divide-cactus-border">
      <div className="px-5 py-2.5">
        <div className="text-xs font-semibold uppercase tracking-wider text-cactus-text-muted">
          {i18n._(/* i18n */ { id: 'You Spend' })}
        </div>
        <div className="mt-1.5 flex flex-col gap-1.5">
          {walletDelta.spending.length === 0 ? (
            <span className="text-sm text-cactus-text-secondary">{i18n._(/* i18n */ { id: 'Nothing' })}</span>
          ) : (
      ...
      {walletDelta.fee !== undefined && (
        <div className="px-5 py-2.5">
          ...
          <div className="mt-0.5 text-sm font-medium text-cactus-text">
...
export default function Confirm(props: ConfirmProps) {
  ...
  const confirmButtonClasses = destructive
    ? 'bg-cactus-danger hover:brightness-110 text-white border-transparent'
    : 'bg-cactus-primary hover:bg-cactus-primary-hover text-[#0f252a] border-transparent';
  ...
  return (
    <div className="flex flex-col h-screen bg-cactus-bg text-cactus-text text-base">
      ...
      <SandboxedIframe className="flex-1 min-h-0 w-full border-0" isDarkMode={isDarkMode}>
        ...
        <div className="h-screen overflow-y-auto px-7 pt-4 pb-4 space-y-3 text-cactus-text font-sans text-base">
          <div className="flex items-start gap-4">
            <div
              className={`shrink-0 w-12 h-12 rounded-xl flex items-center justify-center ${
                destructive ? 'bg-cactus-danger/15 text-cactus-danger' : 'bg-cactus-primary-soft text-cactus-primary'
              }`}
            >
              <svg
            ...
            <div className="flex-1 min-w-0">
              <h1 className="m-0 text-2xl font-semibold leading-tight text-cactus-text">{title}</h1>
              <p className="mt-0.5 mb-0 text-sm leading-snug text-cactus-text-secondary">{message}</p>
            </div>
          </div>

          {pair && (
            <div className="flex items-start gap-3 px-4 py-2.5 rounded-xl border border-cactus-border bg-cactus-primary-soft">
              <div
                className="shrink-0 w-9 h-9 rounded-md bg-cactus-primary/20"
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline gap-2 min-w-0">
                  ...
                  <span className="text-sm font-semibold truncate text-cactus-text">{pair.metadata.name}</span>
                </div>
                {isDisplayableUrl(pair.metadata.url) && (
                  <div className="text-xs text-cactus-text-secondary truncate">{pair.metadata.url}</div>
                )}
                {pair.metadata.description && (
                  <div className="mt-1 text-xs text-cactus-text-secondary truncate">{pair.metadata.description}</div>
                )}
              </div>
            </div>
          )}
          ...
          {isDifferentFingerprint && (
            <div className="flex items-start gap-3 px-4 py-3 rounded-xl border border-cactus-warning/30 bg-cactus-warning/10 text-cactus-warning">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
          ...
          {!!command && (
            <section className="rounded-xl border border-cactus-border bg-cactus-card px-5 py-3">
              <div className="text-xs font-semibold uppercase tracking-wider text-cactus-text-muted">Command</div>
              <div className="mt-1 text-sm font-mono break-all text-cactus-text">{command}</div>
            </section>
          )}
            ...
            <section className="rounded-xl border border-cactus-border bg-cactus-card overflow-hidden divide-y divide-cactus-border">
              {rows.map(({ field, label, value }) => (
                <div className="px-5 py-2.5" key={field}>
                  <div className="text-xs font-semibold uppercase tracking-wider text-cactus-text-muted">{label}</div>
                  <div className="mt-0.5 text-sm font-medium break-all whitespace-pre-wrap text-cactus-text">{value}</div>
                </div>
              ))}
            </section>
          )}
          ...
          {hasData && (
            <Collapsible title="Raw data">
              <pre className="m-0 text-xs font-mono leading-relaxed break-all whitespace-pre-wrap text-cactus-text-secondary">
      ...
      <div className="flex justify-between items-center gap-2.5 px-7 py-4 border-t border-cactus-border bg-cactus-bg">
        {showBypassToggle ? (
          <label className="flex items-center gap-2 text-xs text-cactus-text-secondary cursor-pointer select-none">
            <input
              type="checkbox"
              data-form-field="rememberBypass"
              className="w-[16px] h-[16px] accent-cactus-primary cursor-pointer"
            />
        ...
        <div className="flex items-center gap-2.5">
          <button
            type="button"
            data-action="cancel"
            className="h-9 px-5 text-sm font-semibold uppercase tracking-wider rounded-md border border-cactus-primary bg-transparent text-cactus-primary hover:bg-cactus-primary-soft transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-cactus-primary"
          >
          ...
          <button
            ...
            className={`h-9 px-5 text-sm font-semibold uppercase tracking-wider rounded-md border shadow-sm transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-cactus-primary focus-visible:ring-offset-2 focus-visible:ring-offset-cactus-bg ${confirmButtonClasses}`}
          >
            {confirmLabel}
          </button>
        </div>
