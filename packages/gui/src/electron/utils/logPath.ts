...
/**
 * Returns the default Cactus debug log path derived from CACTUS_ROOT (or the
 * standard `~/.cactus/mainnet` location). This is the only log path used when the
...
export function getDefaultLogPath(): string {
  ...
  return path.join(cactusRoot, 'log', 'debug.log');
}
 ...
 * be set through the native file picker in `CactusLogsAPI.SET_PATH`), the value on
