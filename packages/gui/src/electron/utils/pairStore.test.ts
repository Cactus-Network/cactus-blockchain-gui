...
describe('pairStore bypass reset', () => {
  it('adds a bypass command to a pair and refreshes updatedAt', () => {
    jest.spyOn(Date, 'now').mockReturnValue(2000);
    seedPairs([makePair({ bypass: ['cactus_takeOffer'] })]);
    const store = loadStore();

    store.addBypassCommand('topic-1', 'cactus_sendTransaction');

    expect(store.findPair('topic-1')).toMatchObject({
      bypass: ['cactus_takeOffer', 'cactus_sendTransaction'],
      updatedAt: 2000,
    });
  });

  it('does not duplicate an existing bypass command', () => {
    seedPairs([makePair({ bypass: ['cactus_takeOffer'], updatedAt: 100 })]);
    const store = loadStore();

    store.addBypassCommand('topic-1', 'cactus_takeOffer');
  ...
  it('throws when adding a bypass command for a missing pair', () => {
    ...
    expect(() => store.addBypassCommand('missing', 'cactus_takeOffer')).toThrow('Pair not found: missing');
  });
  ...
  it('clears bypass permissions for all pairs', () => {
    ...
    seedPairs([
      makePair({ topic: 'topic-1', bypass: ['cactus_takeOffer'] }),
      makePair({ topic: 'topic-2', bypass: ['cactus_sendTransaction'] }),
    ]);
    const store = loadStore();
