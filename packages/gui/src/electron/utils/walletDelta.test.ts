...
describe('createOfferToWalletDelta', () => {
  ...
  it('rejects cac keys in create-offer params because create offers use numeric wallet IDs', () => {
    expect(() =>
      createOfferToWalletDelta({
        cac: '-4',
      }),
    ).toThrow('CAC is not a valid asset ID');
  });
...
describe('offerSummaryToWalletDelta', () => {
  it('converts a production-shaped CAC-for-CAT offer summary into taker wallet perspective', () => {
  ...
  it('allows summaries that include cac in both offered and requested maps as separate wallet directions', () => {
    expect(
  ...
  it('rejects numeric CAC asset keys in offer summaries', () => {
    expect(() =>
      offerSummaryToWalletDelta({
        offered: { '1': '4' },
        requested: {},
  ...
  it('converts requested CAC when offered does not also contain cac', () => {
